...
import net.lizhaoweb.spring.mybatis.actable.annotation.IgnoreUpdate;
import net.lizhaoweb.spring.mybatis.actable.annotation.Index;
import net.lizhaoweb.spring.mybatis.actable.annotation.Table;
import net.lizhaoweb.spring.mybatis.actable.annotation.Unique;
import net.lizhaoweb.spring.mybatis.actable.command.*;
import net.lizhaoweb.spring.mybatis.actable.constants.Constants;
import net.lizhaoweb.spring.mybatis.actable.constants.MySqlCharsetConstant;
import net.lizhaoweb.spring.mybatis.actable.constants.MySqlEngineConstant;
import net.lizhaoweb.spring.mybatis.actable.constants.MySqlTypeConstant;
import net.lizhaoweb.spring.mybatis.actable.dao.system.CreateMysqlTablesMapper;
import net.lizhaoweb.spring.mybatis.actable.manager.util.ConfigurationUtil;
import net.lizhaoweb.spring.mybatis.actable.utils.ClassScaner;
import net.lizhaoweb.spring.mybatis.actable.utils.ClassTools;
import net.lizhaoweb.spring.mybatis.actable.utils.ColumnUtils;
...
public class SysMysqlCreateTableManagerImpl implements SysMysqlCreateTableManager {
    ...
    private void buildTableMapConstruct(Class<?> clas, Map<String, Map<String, TableConfig>> baseTableMap) {
        ...
        // 1. 用于存表的全部字段
        List<CreateTableParam> allFieldList;
        try {
        ...
        // 2. 找出增加的字段
        List<CreateTableParam> addFieldList = getAddFieldList(allFieldList, columnNames);

        // 3. 找出删除的字段
        List<CreateTableParam> removeFieldList = getRemoveFieldList(columnNames, allFieldList);

        // 4. 找出更新的字段
        List<CreateTableParam> modifyFieldList = getModifyFieldList(tableColumnList, allFieldList);

        // 5. 找出需要删除主键的字段
        List<CreateTableParam> dropKeyFieldList = getDropKeyFieldList(tableColumnList, allFieldList);
        ...
        // 6. 找出需要删除的索引和唯一约束
        List<CreateTableParam> dropIndexAndUniqueFieldList = getDropIndexAndUniqueList(allIndexAndUniqueNames, allFieldList);

        // 7. 找出需要新增的索引
        List<CreateTableParam> addIndexFieldList = getAddIndexList(allIndexAndUniqueNames, allFieldList);

        // 8. 找出需要新增的唯一约束
        List<CreateTableParam> addUniqueFieldList = getAddUniqueList(allIndexAndUniqueNames, allFieldList);
     ...
     * @return 需要新建的索引
     */
    private List<CreateTableParam> getAddIndexList(Set<String> allIndexAndUniqueNames, List<CreateTableParam> allFieldList) {
        List<CreateTableParam> addIndexFieldList = new ArrayList<CreateTableParam>();
        if (null == allIndexAndUniqueNames) {
            allIndexAndUniqueNames = new HashSet<String>();
        }
        for (CreateTableParam createTableParam : allFieldList) {
     ...
     * @return 需要新建的唯一约束
     */
    private List<CreateTableParam> getAddUniqueList(Set<String> allIndexAndUniqueNames, List<CreateTableParam> allFieldList) {
        List<CreateTableParam> addUniqueFieldList = new ArrayList<CreateTableParam>();
        if (null == allIndexAndUniqueNames) {
            allIndexAndUniqueNames = new HashSet<String>();
        }
        for (CreateTableParam createTableParam : allFieldList) {
     ...
     * @return 需要删除的索引和唯一约束
     */
    private List<CreateTableParam> getDropIndexAndUniqueList(Set<String> allIndexAndUniqueNames, List<CreateTableParam> allFieldList) {
        List<CreateTableParam> dropIndexAndUniqueFieldList = new ArrayList<CreateTableParam>();
        ...
        for (String string : allIndexAndUniqueNames) {
            if (!currentModelIndexAndUnique.contains(string)) {
                ...
                createTableParam.setFiledIndexName(string);
                dropIndexAndUniqueFieldList.add(createTableParam);
            }
        }
        return dropIndexAndUniqueFieldList;
    }
     ...
     * @return 需要删除主键的字段
     */
    private List<CreateTableParam> getDropKeyFieldList(List<SysMysqlColumns> tableColumnList, List<CreateTableParam> allFieldList) {
     ...
     * @return 需要修改的字段
     */
    private List<CreateTableParam> getModifyFieldList(List<SysMysqlColumns> tableColumnList, List<CreateTableParam> allFieldList) {
        Map<String, CreateTableParam> fieldMap = getAllFieldMap(allFieldList);
     ...
     *
     * @param allFieldList a
     * @return 返回
     */
    private Map<String, CreateTableParam> getAllFieldMap(List<CreateTableParam> allFieldList) {
     ...
     * @param allFieldList model中的所有字段
     */
    private List<CreateTableParam> getRemoveFieldList(List<String> columnNames, List<CreateTableParam> allFieldList) {
//        List<String> toLowerCaseColumnNames = ClassTools.toLowerCase(columnNames);
        List<String> trimColumnNames = ClassTools.trim(columnNames);
        Map<String, CreateTableParam> fieldMap = getAllFieldMap(allFieldList);
        // 用于存删除的字段
        List<CreateTableParam> removeFieldList = new ArrayList<CreateTableParam>();
        for (String fieldNm : trimColumnNames) {
            // 判断该字段在新的model结构中是否存在
            if (fieldMap.get(fieldNm) == null) {
                CreateTableParam removeField = new CreateTableParam();
                removeField.setFieldName(fieldNm);
                // 不存在，做删除处理
                removeFieldList.add(removeField);
            }
        }
     ...
     * @return 新增的字段
     */
    private List<CreateTableParam> getAddFieldList(List<CreateTableParam> allFieldList, List<String> columnNames) {
//        List<String> toLowerCaseColumnNames = ClassTools.toLowerCase(columnNames);
        List<String> trimColumnNames = ClassTools.trim(columnNames);
        List<CreateTableParam> addFieldList = new ArrayList<CreateTableParam>();
        for (CreateTableParam createTableParam : allFieldList) {
            // 循环新的model中的字段，判断是否在数据库中已经存在
            if (!trimColumnNames.contains(createTableParam.getFieldName().toLowerCase())) {
                // 不存在，表示要在数据库中增加该字段
     ...
     */
    @Override
    public List<CreateTableParam> getAllFields(Class<?> clas) {
        String idxPrefix = springContextUtil.getConfig(Constants.TABLE_INDEX_KEY);
        String uniPrefix = springContextUtil.getConfig(Constants.TABLE_UNIQUE_KEY);
        List<CreateTableParam> fieldList = new ArrayList<CreateTableParam>();
...
//                    param.setFiledIndexName((index.value() == null || index.value().equals(""))
                    param.setFiledIndexName(index.value().equals("")
...
//                    param.setFiledUniqueName((unique.value() == null || unique.value().equals(""))
                    param.setFiledUniqueName(unique.value().equals("")
                            ? uniPrefix
    ...
    private void dropIndexAndUniqueByMap(Map<String, TableConfig> dropIndexAndUniqueMap) {
        if (dropIndexAndUniqueMap.size() > 0) {
            for (Entry<String, TableConfig> entry : dropIndexAndUniqueMap.entrySet()) {
                for (CreateTableParam dropIndexAndUnique : entry.getValue().getList()) {
                    Map<String, CreateTableParam> map = new HashMap<String, CreateTableParam>();
                    map.put(entry.getKey(), dropIndexAndUnique);
                    log.info("开始删除表" + entry.getKey() + "中的索引" + dropIndexAndUnique.getFiledIndexName());
                    createMysqlTablesMapper.dropTabelIndex(map);
                    log.info("完成删除表" + entry.getKey() + "中的索引" + dropIndexAndUnique.getFiledIndexName());
                }
            }
        }
    }
    ...
    private void addIndexByMap(Map<String, TableConfig> addIndexMap) {
        if (addIndexMap.size() > 0) {
            for (Entry<String, TableConfig> entry : addIndexMap.entrySet()) {
                for (CreateTableParam fieldProperties : entry.getValue().getList()) {
                    Map<String, CreateTableParam> map = new HashMap<String, CreateTableParam>();
                    map.put(entry.getKey(), fieldProperties);
        ...
        if (removeTableMap.size() > 0) {
            for (Entry<String, TableConfig> entry : removeTableMap.entrySet()) {
                for (CreateTableParam fieldProperties : entry.getValue().getList()) {
                    Map<String, CreateTableParam> map = new HashMap<String, CreateTableParam>();
                    map.put(entry.getKey(), fieldProperties);
                    log.info("开始删除表" + entry.getKey() + "中的字段" + fieldProperties.getFieldName());
                    createMysqlTablesMapper.removeTableField(map);
                    log.info("完成删除表" + entry.getKey() + "中的字段" + fieldProperties.getFieldName());
                }
            }
        }
    }
