 ...
 *
 * @project : spring
 * @package : net.lizhaoweb.quentin.security
 * @date : 2024-06-11
 * @time : 09:31
 */
package net.lizhaoweb.quentin.security;
 ...
 * <p>
 * Created by jhon on 2024/6/11 9:31
 *
...
public final class SecurityUtil {
     ...
     */
    private static HashMap<Object, Method[]> objectCache = new HashMap<Object, Method[]>();

    private static String PACKAGE = "net.lizhaoweb.quentin.security";
     ...
     *                        privilege apply..
     */
    public static void doAsPrivilege(final String methodName, final Servlet targetObject, final Class<?>[] targetType, final Object[] targetArguments, Principal principal) throws Exception {
        Method method = null;
        ...
        if (objectCache.containsKey(targetObject)) {
            ...
            if (method == null) {
                method = createMethodAndCacheIt(methodsCache, methodName, targetObject, targetType);
            }
        } else {
            method = createMethodAndCacheIt(methodsCache, methodName, targetObject, targetType);
        }
     ...
     *                        privilege apply
     */
    public static void doAsPrivilege(final String methodName, final Filter targetObject, final Class<?>[] targetType, final Object[] targetArguments, Principal principal) throws Exception {
        Method method = null;
        Method[] methodsCache = null;
        if (objectCache.containsKey(targetObject)) {
            methodsCache = objectCache.get(targetObject);
    ...
    private static Method findMethod(Method[] methodsCache, String methodName) {
        if (methodName.equalsIgnoreCase(INIT_METHOD) && methodsCache[INIT] != null) {
            return methodsCache[INIT];
        } else if (methodName.equalsIgnoreCase(DESTROY_METHOD) && methodsCache[DESTROY] != null) {
            return methodsCache[DESTROY];
        } else if (methodName.equalsIgnoreCase(SERVICE_METHOD) && methodsCache[SERVICE] != null) {
            return methodsCache[SERVICE];
        } else if (methodName.equalsIgnoreCase(DOFILTER_METHOD) && methodsCache[DOFILTER] != null) {
            return methodsCache[DOFILTER];
        } else if (methodName.equalsIgnoreCase(EVENT_METHOD) && methodsCache[EVENT] != null) {
            return methodsCache[EVENT];
        } else if (methodName.equalsIgnoreCase(DOFILTEREVENT_METHOD) && methodsCache[DOFILTEREVENT] != null) {
