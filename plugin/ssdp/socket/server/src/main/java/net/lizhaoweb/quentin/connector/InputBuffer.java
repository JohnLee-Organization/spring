...
import net.lizhaoweb.mikefox.buf.B2CConverter;
 ...
 * BufferedReader).
 * <p>
 * Created by jhon on 2024/6/11 9:11
 *
...
public class InputBuffer extends Reader implements ByteChunk.ByteInputChannel, CharChunk.CharInputChannel, CharChunk.CharOutputChannel {
    ...
    public static final String DEFAULT_ENCODING = net.lizhaoweb.coyote.Constants.DEFAULT_CHARACTER_ENCODING;
    ...
    public int realReadBytes(byte[] cbuf, int off, int len) throws IOException {
        if (closed) return -1;
        if (coyoteRequest == null) return -1;
        if (state == INITIAL_STATE) state = BYTE_STATE;
    ...
    public int realReadChars(char[] cbuf, int off, int len) throws IOException {
        ...
        if (cb.getLimit() < limit) cb.setLimit(limit);
        state = CHAR_STATE;
    ...
    protected void setConverter() throws IOException {
        if (coyoteRequest != null) enc = coyoteRequest.getCharacterEncoding();
        gotEnc = true;
