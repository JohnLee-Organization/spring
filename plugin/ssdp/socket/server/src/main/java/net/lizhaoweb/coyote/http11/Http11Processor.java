...
import net.lizhaoweb.coyote.*;
import net.lizhaoweb.mikefox.buf.ByteChunk;
import net.lizhaoweb.mikefox.buf.HexUtils;
import net.lizhaoweb.mikefox.buf.MessageBytes;
import net.lizhaoweb.mikefox.http.FastHttpDateFormat;
 ...
 * <p>
 * Created by jhon on 2024/6/7 10:38
 *
...
public class Http11Processor extends AbstractHttp11Processor implements ActionHook {
    ...
    public Http11Processor(int headerBufferSize, JIoUdpEndpoint endpoint) {
     ...
     */
    protected SocketWrapper<MulticastSocket> socket;


    /**
     * Associated endpoint.
     */
    protected JIoUdpEndpoint endpoint;
     ...
     */
    public SocketState process(SocketWrapper<MulticastSocket> socketWrapper) throws IOException {
        RequestInfo rp = request.getRequestProcessor();
        rp.setStage(net.lizhaoweb.coyote.Constants.STAGE_PARSE);
...
//        outputBuffer.setOutputStream(socket.getSocket().getOutputStream());
        inputBuffer.setInputStream(new ByteArrayInputStream(socket.getDatagramPacket().getData()));
        outputBuffer.setOutputStream(new ByteArrayOutputStream());
        ...
        while (started && !error && keepAlive) {
            ...
            if (!error) {
                ...
                rp.setStage(net.lizhaoweb.coyote.Constants.STAGE_PREPARE);
                try {
                    prepareRequest();
                } catch (Throwable t) {
                    if (log.isDebugEnabled()) {
            ...
            if (!error) {
                try {
                    rp.setStage(net.lizhaoweb.coyote.Constants.STAGE_SERVICE);
            ...
            try {
                rp.setStage(net.lizhaoweb.coyote.Constants.STAGE_ENDINPUT);
                // If we know we are closing the connection, don't drain input.
                // This way uploading a 100GB file doesn't tie up the thread
                // if the servlet has rejected it.

                if (error && !async) inputBuffer.setSwallowInput(false);
                if (!async) endRequest();
            } catch (Throwable t) {
            ...
            try {
                rp.setStage(net.lizhaoweb.coyote.Constants.STAGE_ENDOUTPUT);
            } catch (Throwable t) {
            ...
            rp.setStage(net.lizhaoweb.coyote.Constants.STAGE_KEEPALIVE);
        ...
        rp.setStage(net.lizhaoweb.coyote.Constants.STAGE_ENDED);
        if (error) {
            recycle();
    ...
    public void action(ActionCode actionCode, Object param) {
        if (actionCode == ActionCode.ACTION_COMMIT) {
            ...
            outputBuffer.reset();
//TODO        } else if (actionCode == ActionCode.ACTION_CUSTOM) {
//            // Do nothing
...
//TODO        } else if (actionCode == ActionCode.ACTION_REQ_SET_BODY_REPLAY) {
...
//            internalBuffer.addActiveFilter(savedBody);
            ...
            RequestInfo rp = request.getRequestProcessor();
            if (rp.getStage() != net.lizhaoweb.coyote.Constants.STAGE_SERVICE) { //async handling
                dispatch.set(true);
    ...
    protected void prepareRequest() {
        ...
        if ((restrictedUserAgents != null) && ((http11) || (keepAlive))) {
            ...
            if (userAgentValueMB != null) {
                ...
                for (java.util.regex.Pattern restrictedUserAgent : restrictedUserAgents) {
                    assert userAgentValue != null;
                    if (restrictedUserAgent.matcher(userAgentValue).matches()) {
                        http11 = false;
                        keepAlive = false;
                        break;
                    }
        ...
        if (transferEncodingValueMB != null) {
            ...
            int startPos = 0;
            assert transferEncodingValue != null;
...
//TODO        inputBuffer.addFilter(new IdentityInputFilter());
...
//        outputBuffer.addFilter(new GzipOutputFilter());
    ...
    protected void parseHost(MessageBytes valueMB) {
        ...
        if (colonPos < 0) {
            // 1900 - Default HTTP port
            request.setServerPort(1900);
