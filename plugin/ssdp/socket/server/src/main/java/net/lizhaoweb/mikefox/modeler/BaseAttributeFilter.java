 ...
 * <p>
 * Created by jhon on 2024/6/7 19:49
 *
...
public class BaseAttributeFilter implements NotificationFilter {
    ...
    public BaseAttributeFilter(String name) {
        super();
        if (name != null) addAttribute(name);
    }
    ...
    public boolean isNotificationEnabled(Notification notification) {
        ...
        if (!AttributeChangeNotification.ATTRIBUTE_CHANGE.equals(acn.getType())) return (false);
        synchronized (names) {
            if (names.size() < 1) return (true);
            else return (names.contains(acn.getAttributeName()));
        }
    }
