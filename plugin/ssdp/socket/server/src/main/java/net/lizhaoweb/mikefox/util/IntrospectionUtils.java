 ...
 *
 * @project : spring
 * @package : net.lizhaoweb.mikefox.util
 * @date : 2024-06-06
 * @time : 17:30
 */
package net.lizhaoweb.mikefox.util;
 ...
 * <p>
 * Created by jhon on 2024/6/6 17:30
 *
...
//    private static final org.apache.juli.logging.Log log = org.apache.juli.logging.LogFactory.getLog(IntrospectionUtils.class);
    ...
    public static void setAttribute(Object proxy, String n, Object v) throws Exception {
        ...
        if (log.isDebugEnabled()) log.debug("Setting " + n + "=" + v + "  in " + proxy);
    ...
    public static ClassLoader getURLClassLoader(URL[] urls, ClassLoader parent) {
        try {
            ...
            return (ClassLoader) m.invoke(urlCL, new Object[]{urls, parent});
        ...
        while (st.hasMoreTokens()) {
            ...
            if (path.endsWith(jarName)) {
                ...
                try {
                    ...
                    if (parentDir == null) parentDir = home; // unix style
                ...
                if (new File(fname).exists()) {
                    try {
                        ...
                        String parentDir = f.getParent();
                        if (parentDir == null) parentDir = path; // unix style
                        File f1 = new File(parentDir);
                        install = f1.getCanonicalPath();
                        if (installSysProp != null) System.getProperties().put(installSysProp, install);
                        if (home == null && homeSysProp != null) System.getProperties().put(homeSysProp, install);
    ...
    public static String classPathAdd(URL[] urls, String cp) {
        ...
        if (cp == null) {
            isNull = true;
            cpBuilder = new StringBuilder();
        } else {
            cpBuilder = new StringBuilder(cp);
        }
        for (URL url : urls) {
            if (isNull) {
                cpBuilder = new StringBuilder(url.getFile());
                isNull = false;
            } else cpBuilder.append(PATH_SEPARATOR).append(url.getFile());
        }
    ...
    public static boolean setProperty(Object o, String name, String value, boolean invokeSetProperty) {
        ...
        try {
            ...
            for (Method method : methods) {
                ...
                if (setter.equals(method.getName()) && paramT.length == 1 && "java.lang.String".equals(paramT[0].getName())) {
    ...
    public static Object getProperty(Object o, String name) {
        ...
        try {
            ...
            if (log.isDebugEnabled()) log.debug("IntrospectionUtils: SecurityException for " + o.getClass() + " " + name + ")", ex1);
        } catch (IllegalAccessException iae) {
            if (log.isDebugEnabled()) log.debug("IntrospectionUtils: IllegalAccessException for " + o.getClass() + " " + name + ")", iae);
        } catch (InvocationTargetException ie) {
            if (log.isDebugEnabled()) log.debug("IntrospectionUtils: InvocationTargetException for " + o.getClass() + " " + name + ")");
        }
        return null;
    }
    ...
    public static void setProperty(Object o, String name) {
        ...
        try {
            ...
            if (log.isDebugEnabled()) log.debug("IntrospectionUtils: Exception for " + o.getClass() + " " + name, ex1);
        }
    }
    ...
    public static Method findMethod(Class<?> c, String name, Class<?>[] params) {
        ...
        for (Method method : methods) {
            if (method.getName().equals(name)) {
                ...
                if (methodParams == null) if (params == null || params.length == 0) return method;
                if (params == null) if (methodParams == null || methodParams.length == 0) return method;
                assert params != null;
    ...
    public static Object callMethod0(Object target, String methodN) throws Exception {
        if (target == null) {
            if (log.isDebugEnabled()) log.debug("IntrospectionUtils: Assert: Illegal params null");
            return null;
        }
