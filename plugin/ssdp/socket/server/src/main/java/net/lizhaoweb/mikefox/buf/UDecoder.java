 ...
 * <p>
 * Created by jhon on 2024/6/7 14:56
 *
...
public final class UDecoder {
    ...
    private static final boolean ALLOW_ENCODED_SLASH = Boolean.parseBoolean(System.getProperty("org.apache.tomcat.util.buf.UDecoder.ALLOW_ENCODED_SLASH", "false"));
    ...
    public void convert(ByteChunk mb, boolean query) throws IOException {
        ...
        int idx2 = -1;
        if (query) idx2 = ByteChunk.indexOf(buff, start, end, '+');
        if (idx < 0 && idx2 < 0) {
            return;
        }
        ...
        for (int j = idx; j < end; j++, idx++) {
            if (buff[j] == '+' && query) {
                ...
                byte b1 = buff[j + 1];
                byte b2 = buff[j + 2];
                if (!isHexDigit(b1) || !isHexDigit(b2)) throw new CharConversionException("isHexDigit");
    ...
    public void convert(CharChunk mb, boolean query) throws IOException {
        ...
        int idx2 = -1;
        if (query) idx2 = CharChunk.indexOf(buff, start, cend, '+');
        if (idx < 0 && idx2 < 0) {
            return;
        }
    ...
    public final String convert(String str, boolean query) {
        ...
        if ((!query || str.indexOf('+') < 0) && str.indexOf('%') < 0) return str;
