 ...
 * @Title : ShortArrayResultTypeHandler.java
...
public class ShortArrayResultTypeHandler extends AbstractString2ArrayResultTypeHandler<Short> {

	@Override
	protected Short[] getTargetArray(String columnValue) {
		if (columnValue == null) {
			return null;
		}
		...
		for (String numberString : columnValue.split(",")) {
			if (numberString.matches("^\\d+$")) {
				try {
					result.add(Short.valueOf(numberString));
				} catch (Exception e) {
				}
			}
		}
