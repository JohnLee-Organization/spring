        ...
        if (CharacterUtil.CHAR_USELESS != context.getCurrentCharType()) {
            ...
            if (!this.tmpHits.isEmpty()) {
                ...
                for (Hit hit : tmpArray) {
                    ...
                    if (hit.isMatch()) {
                        //输出当前的词
                        Lexeme newLexeme = new Lexeme(context.getBufferOffset(), hit.getBegin(), context.getCursor() - hit.getBegin() + 1, LexemeType.CNWORD);
            ...
            if (singleCharHit.isMatch()) {//首字成词
                //输出当前的词
                Lexeme newLexeme = new Lexeme(context.getBufferOffset(), context.getCursor(), 1, LexemeType.CNWORD);
                context.addLexeme(newLexeme);
