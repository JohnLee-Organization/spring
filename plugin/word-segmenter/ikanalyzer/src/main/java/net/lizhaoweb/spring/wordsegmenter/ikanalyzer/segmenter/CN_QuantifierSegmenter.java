...
public class CN_QuantifierSegmenter implements ISegmenter {
     ...
     * @param context 上下文
     */
    private void processCNumber(IAnalyzeContext context) {
     ...
     * @param context 上下文
     */
    private void processCount(IAnalyzeContext context) {
        // 判断是否需要启动量词扫描
        ...
        if (CharacterUtil.CHAR_CHINESE == context.getCurrentCharType()) {
            ...
            if (!this.countHits.isEmpty()) {
                ...
                for (Hit hit : tmpArray) {
                    ...
                    if (hit.isMatch()) {
                        //输出当前的词
                        Lexeme newLexeme = new Lexeme(context.getBufferOffset(), hit.getBegin(), context.getCursor() - hit.getBegin() + 1, LexemeType.COUNT);
            ...
            if (singleCharHit.isMatch()) {//首字成量词词
                //输出当前的词
                Lexeme newLexeme = new Lexeme(context.getBufferOffset(), context.getCursor(), 1, LexemeType.COUNT);
                context.addLexeme(newLexeme);
     ...
     * @return boolean
     */
    private boolean needCountScan(IAnalyzeContext context) {
        if ((nStart != -1 && nEnd != -1) || !countHits.isEmpty()) {
            ...
            if (!context.getOrgLexemes().isEmpty()) {
                ...
                if (LexemeType.CNUM == l.getLexemeType() || LexemeType.ARABIC == l.getLexemeType()) {
     ...
     * @param context 上下文
     */
    private void outputNumLexeme(IAnalyzeContext context) {
        if (nStart > -1 && nEnd > -1) {
            //输出数词
            Lexeme newLexeme = new Lexeme(context.getBufferOffset(), nStart, nEnd - nStart + 1, LexemeType.CNUM);
            context.addLexeme(newLexeme);
        }
    }
}
